  ...
  useEffect(() => {
    const fetchApplication = async () => {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE}/api/applications/by-formid/${encodedFormId}`, {
          method: 'GET',
          headers: {
      ...
      <View style={{ flex: 1, backgroundColor: '#fff' }}>
        ...
        <ScrollView contentContainerStyle={{ paddingBottom: 30 }}>
          ...
          <Text style={styles.useridRow}>
            ...
            <Text> | </Text>
            <Text style={{ color: '#222' }}>Name: <Text style={{ fontWeight: 'bold' }}>{application.applicant?.name || application.applicant?.applicant?.name || 'N/A'}</Text></Text>
          </Text>
            ...
            {activeTab === 'personal' ? (
              <>
                ...
                <View style={styles.detailRow}>
                  ...
                  <Text style={styles.detailValue}>{application.applicant?.name || application.applicant?.applicant?.name || 'N/A'}</Text>
                </View>
                <View style={styles.detailRow}>
                  <Text style={styles.detailLabel}>Father's Name</Text>
                  <Text style={styles.detailValue}>{application.applicant?.fatherName || application.applicant?.applicant?.fatherName || 'N/A'}</Text>
                </View>
                <View style={styles.detailRow}>
                  <Text style={styles.detailLabel}>DOB</Text>
                  <Text style={styles.detailValue}>{new Date(application.applicant?.dateOfBirth || application.applicant?.applicant?.dateOfBirth).toLocaleDateString()}</Text>
                </View>
                <View style={styles.detailRow}>
                  <Text style={styles.detailLabel}>Aadhar No</Text>
                  <Text style={styles.detailValue}>{application.applicant?.aadharNo || application.applicant?.applicant?.aadharNo || 'N/A'}</Text>
                </View>
              </>
            ) : (
              <>
